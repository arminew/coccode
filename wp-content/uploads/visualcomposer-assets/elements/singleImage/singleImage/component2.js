...
export default class SingleImageElement2 extends vcvAPI.elementComponent {
  static drawImageProp (ctx, img, x, y, w, h, offsetX, offsetY) {
    ...
    offsetX = typeof offsetX === 'number' ? offsetX : .5
    offsetY = typeof offsetY === 'number' ? offsetY : .5
  ...
  componentWillUnmount () {
    SingleImageElement2.image && SingleImageElement2.image.removeEventListener('load', this.setImageState)
  }
  ...
  setImage (props) {
    ...
    SingleImageElement2.image = new Image()

    SingleImageElement2.image.addEventListener('load', this.setImageState)

    SingleImageElement2.image.src = imgSrc
  ...
  resizeImage () {
    ...
    SingleImageElement2.drawImageProp(ctx, imgElement, 0, 0, parsedWidth, parsedHeight, .5, .5)
  }
  ...
  render () {
    ...
    if (image && image.filter && image.filter !== 'normal') {
      classes += ` vce-image-filter--${image.filter}`
    }
